...
export async function submitConciergeRequest(
  ...
  if (!parsed.success) {
    if (parsed.error.issues.some((i) => i.path[0] === "website")) {
      return { ok: true, message: "Thank you. Our concierge team will reach out." };
    }
  ...
  if (!(await rateLimit("concierge", ip, 5, 600)).ok) {
    return { ok: false, message: "Too many requests. Please try again shortly." };
  }

  const d = parsed.data;
  const composed = [
    `Festival dates: ${d.datesNeeded}`,
    `Party size: ${d.partySize || "n/a"}`,
    `Package interest: ${d.tier || "n/a"}`,
    "",
  ...
  try {
    ...
    await sendLeadNotification("New concierge request", [
      `Name: ${d.visitorName}`,
    ...
    console.error("submitConciergeRequest failed", err);
    return { ok: false, message: "Something went wrong on our end. Please try again." };
  }

  return {
    ok: true,
    message: "Thank you. Our concierge team will reach out to plan your festival.",
  };
}
