...
import { submitUrgentStay } from "@/actions/submit-urgent";
import type { FormState } from "@/actions/submit-lead";
import { FormStatus } from "./FormStatus";

const initial: FormState = { ok: false };

export function UrgentStayForm() {
  const [state, action, pending] = useActionState(submitUrgentStay, initial);
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <Field label="Dates you need" name="datesNeeded" error={state.errors?.datesNeeded} required />
      <Field label="Party size" name="partySize" error={state.errors?.partySize} />
      <Field label="Budget" name="budget" error={state.errors?.budget} />
      <div>
